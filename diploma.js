document.addEventListener('DOMContentLoaded', () => {
    ...
    async function finddiploma() {
        ...
        try {
            const fileName = diploma_input + ".pdf"; // Added .pdf extension for safety
            ...
            if (!checkResponse.ok) {
                // Try without .pdf if with .pdf failed
                const fileNameNoExt = diploma_input;
                const { data: dataNoExt } = supabaseClient.storage.from('diplomas').getPublicUrl(fileNameNoExt);
                const checkResponseNoExt = await fetch(dataNoExt.publicUrl, { method: 'HEAD' });

                if (!checkResponseNoExt.ok) {
                    console.warn("File not found on Supabase (public check)");
                    showMessage("Diploma not found for this roll number.", "error");
                    return;
                }

                data.publicUrl = dataNoExt.publicUrl;
            }
